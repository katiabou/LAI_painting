...
rule prep_sample_files:
    """
    Prepare source and target files for LAI methods
    """
    input:
    ...
    output:
        ...
        meta_source_name = 'output/seed_{seed}/files/source_name_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.txt',
        meta_source_name_1 = 'output/seed_{seed}/files/source_name_1_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.txt',
        meta_source_name_2 = 'output/seed_{seed}/files/source_name_2_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.txt',
        ...
        meta_target_source_name = 'output/seed_{seed}/files/target_source_name_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.txt',
    params:
        source_size = '{source}',
        gen_adm = '{gen_adm}',
        source_1 = 'pop_b',
...
rule flare_gen_map:
    """ 
    Fix the gen map files for flare format (making a dummy uniform rate, dividing the bp / 100000000)
    """
    input:
    ...
    shell:
        '''
        bcftools query -f '%CHROM %CHROM\_%POS %POS\n' {input.sim_vcf_filt} | awk '{{print $1" "$2" "$3/1000000" "$3}}' > {output.gen_map_flare}
        '''

rule flare_simulation:
    """ 
    Run flare on simulated data for null model
    ...
    threads: 4
    log:
        'output/seed_{seed}/flare/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.anc.vcf.gz.log'
    benchmark:
...
rule pop_source1_vcf:
    ...
    input:
        ...
        sim_vcf_filt = 'output/seed_{seed}/slendr/sim_data/model_gf_{gf}_gen_adm_{gen_adm}_filt.vcf.gz',
    output:
        pop_b_vcf = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/pop_b.vcf.gz',
        genotype_file_pop_b = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/pop_bgenofile.1',
    threads: 4
    shell:
        '''        
        bcftools view \
...
rule pop_source2_vcf:
    ...
    input:
        ...
        sim_vcf_filt = 'output/seed_{seed}/slendr/sim_data/model_gf_{gf}_gen_adm_{gen_adm}_filt.vcf.gz',
    output:
        pop_c_vcf = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/pop_c.vcf.gz',
        genotype_file_pop_c = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/pop_cgenofile.1',
    threads: 4
    shell:
        '''
        bcftools view \
...
rule pop_target_vcf:
    ...
    input:
        ...
        sim_vcf_filt = 'output/seed_{seed}/slendr/sim_data/model_gf_{gf}_gen_adm_{gen_adm}_filt.vcf.gz',
    output:
        pop_mix_vcf = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/pop_mix.vcf.gz',
        genotype_file_pop_mix = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/pop_mixgenofile.1',
    threads: 4
    shell:
        '''
        bcftools view \
...
rule sample_list_mosaic:
    ...
    input:
        meta_target_source_pop_name = 'output/seed_{seed}/files/target_source_pop_name_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.txt',
    output:
        samples_mosaic = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/sample.names',
    shell:
        '''
        cp {input.meta_target_source_pop_name} {output.samples_mosaic}
        '''

rule rate_files_mosaic:
    """ 
    ...
    input:
        modern_imputed_snps = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/snpfile.1',
    output:
        temp_sites = temp('output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/temp_sites.1'),
        temp_rate = temp('output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/temp_rate.1'),
        rate_fie = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/rates.1',
    shell:
        '''
...
rule mosaic:
    ...
    params:
        ...
        tmp_dir = '/tmp/seed_{seed}_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}',
        output_dir = 'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/MOSAIC_RESULTS',
        target = 'pop_mix',
        gens = '{gen_adm}'
    benchmark:
        'benchmarks/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.tsv'
    log:
        'output/seed_{seed}/mosaic/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}/run_mosaic.log'
    threads: 6
    shell:
        '''
...
rule rfmix_gen_map:
    """ 
    Fix the gen map files for rfmix format (making a dummy uniform rate, dividing the bp / 100000000)
    """
    input:
        sim_vcf_filt = 'output/seed_{seed}/slendr/sim_data/model_gf_{gf}_gen_adm_{gen_adm}_filt.vcf.gz',
    output:
    ...
    shell:
        '''
        bcftools query -f '%CHROM %POS %POS\n' {input.sim_vcf_filt} | awk '{{print $1" "$2" "$2/1000000}}' > {output.gen_map_rfmix}
        '''

rule prep_target_vcf_rfmix:
    """ 
    ...
    input:
        ...
        meta_target_name = 'output/seed_{seed}/files/target_name_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.txt',
    output:
        target_vcf = 'output/seed_{seed}/rfmix/files/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}_filt-targets.vcf.gz',
    threads: 4
    shell:
        '''
        bcftools view \
...
rule prep_source_vcf_rfmix:
    ...
    input:
        ...
        meta_source_name = 'output/seed_{seed}/files/source_name_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.txt',
    output:
        source_vcf = 'output/seed_{seed}/rfmix/files/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}_filt-source.vcf.gz',
    threads: 4   
    shell:
        '''
        bcftools view \
...
rule run_rfmix:
    ...
    Using the --reanalyze-reference option. In case a set of reference haplotypes may not be of "pure" ancestry and may themselves be somewhat admixed
    """
    input:
        gen_map_rfmix = 'output/seed_{seed}/rfmix/files/model_gf_{gf}_gen_adm_{gen_adm}-genetic_map.txt',
        target_vcf = 'output/seed_{seed}/rfmix/files/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}_filt-targets.vcf.gz',
    ...
    params:
        ...
        prefix = 'output/seed_{seed}/rfmix/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}',
        rfmix_iterations = config['rfmix_iterations'],
        gen = '{gen_adm}'
    benchmark:
        'benchmarks/seed_{seed}/rfmix/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.tsv'
    threads: 6
    shell:
        '''
        {params.rfmix_path} \
...
rule extract_simplai_samples:
    ...
    input:
        ...
        sim_vcf_filt = 'output/seed_{seed}/slendr/sim_data/model_gf_{gf}_gen_adm_{gen_adm}_filt.vcf.gz',
    output:
        simplai_vcf = 'output/seed_{seed}/simplai/files/samples_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.vcf.gz',
        simplai_gen = 'output/seed_{seed}/simplai/output/samples_model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.gen',
    shell:
        '''
...
rule run_simplai:
    ...
    benchmark:
        'benchmarks/seed_{seed}/simplai/output/model_gf_{gf}_source_{source}_gen_adm_{gen_adm}_source_time_{source_time}.tsv'
    shell:
        '''
